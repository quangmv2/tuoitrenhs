...
@section('summary')
	<h1 style="display: none;">{{ cutTitle($post->summary, 40) }}</h1>
@endsection
@section('meta')

	<meta name="Description" content="{{ cutTitle($post->summary, 40) }}">
	<meta name="keywords" content="{{ mb_strtoupper($post->title) }}, QUẬN ĐOÀN NGŨ HÀNH SƠN, Tuổi trẻ Ngũ Hành Sơn, Thanh niên Ngũ Hành Sơn, quandoannguhanhson, quan doan ngu hanh son, quận đoàn ngũ hành sơn, ngũ hành sơn, nguhanhson, ngu hanh son">
			...
			<div class="single_page">
				...
				<div class="post_commentbox"> <a href="{{ route('search') }}?author={{ $post->name_author }}"><i class="fa fa-user"></i>{{ $post->name_author }}</a> <span><i class="fa fa-calendar"></i>{{ $post->created_at->format('d-m-Y H:m:s') }}</span> <a href="{{ route('categoryDetail', ['unsigned_title'=>changeUrl($stack[count($stack)-1])]) }}" title="{{ $stack[count($stack)-1]->name }}"><i class="fa fa-tags"></i>{{ cutTitle($stack[count($stack)-1]->name, 7) }}</a> <span><i class="fa fa-eye" aria-hidden="true"> </i>{{ $post->view }}+</span>
					<div style="display: inline; float: right;">
						...
						<div class="fb-send" 
						    data-href="https{{ route('postDetail',['unsigned_title'=>$post->unsigned_title, 'id'=>$post->id]) }}" 
				...
				<div class="single_page_content"> 
					...
					<hr>
					<div class="others" style="text-transform: uppercase;">
						<ul class="list-other">
