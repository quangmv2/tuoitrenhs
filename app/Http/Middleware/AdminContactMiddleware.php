...
class AdminContactMiddleware
{
    ...
    public function handle($request, Closure $next)
    {
        if ($request->session()->has('account')){
            ...
            $arr = RoleRelationship::where('username', '=', $acc->username)->where('role_id', '=', '5')->get();
            if (count($arr) > 0) {
                return $next($request);
            }
        ...
        exitMD: return redirect()->route('adminHome')->with('notification', 'Bạn không có quyền truy cập chức năng quản lý Liên hệ.');
    }
}
